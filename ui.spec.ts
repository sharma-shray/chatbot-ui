...
test('check if question mark icon exists', async ({ page }) => {
  await page.goto("localhost:3000/*/chat");

  const questionMarkIconDiv = page.locator('div.absolute.bottom-2.right-2.hidden.md\\:block.lg\\:bottom-4.lg\\:right-4');
  await expect(questionMarkIconDiv).toBeVisible();

  const questionMarkIcon = questionMarkIconDiv.locator('svg#icon-question-mark');
  await questionMarkIcon.click();
});

test('check if social media and help links exist', async ({ page }) => {
  await page.goto("localhost:3000/*/chat");

  const twitterLink = page.locator('a[href="https://twitter.com/ChatbotUI"]');
  await expect(twitterLink).toBeVisible();

  // GitHub link exists
  const githubLink = page.locator('a[href="https://github.com/mckaywrigley/chatbot-ui"]');
  await expect(githubLink).toBeVisible();

  // peakerphone link exists
  const speakerphoneLink = page.locator('div.relative.cursor-pointer svg.tabler-icon-speakerphone');
  await expect(speakerphoneLink).toBeVisible();
